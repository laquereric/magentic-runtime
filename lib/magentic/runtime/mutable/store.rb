...
module Magentic
  module Runtime
    module Mutable
      # result.sqlite -- the mutable working data store (the medallion's bronze/silver
      # tiers land here). Ordinary Linux SQLite (NOT OPFS). Implements the PromotionGate
      # sql_reader interface: fetch_row(table, row_id, snapshot:).
      class Store
        ...
        def fetch_row(table, row_id, snapshot: nil)
          pk = pk_col(table)
          @db.execute(%(SELECT * FROM "#{table}" WHERE "#{pk}" = ? LIMIT 1), [row_id.to_s]).first
        ...
        def content_hash(row, columns)
          Array(columns).each_with_object({}) { |c, h| h[c.to_s] = Digest::SHA256.hexdigest(row[c.to_s].to_s) }
        end
        ...
        def normalize(v)
          case v when true then 1 when false then 0 else v end
        end
      end
    end
  end
